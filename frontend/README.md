...
商城/订单状态：PENDING_PAYMENT待支付 / PAID已支付待发货 / SHIPPED已发货 / COMPLETED已完成。
...
| `/consumer/shop` | 在线商城 | 商品卡片（价格/规格/等级/地块/农户），详情弹层选数量+填地址下单，下单后可直接模拟支付；商品溯源码可跳转溯源 |
| `/consumer/orders` | 我的订单 | 需登录；状态步骤条（待支付→已支付待发货→已发货→已完成），待支付可模拟支付、已发货可确认收货，显示物流公司/单号 |
